...
CREATE POLICY "Anyone can upload a result card"
  ON storage.objects FOR INSERT TO anon, authenticated
  WITH CHECK (bucket_id = 'result-cards' AND position('/' in name) = 0);

CREATE POLICY "Result cards are not client readable"
  ON storage.objects FOR SELECT TO anon, authenticated
  USING (bucket_id <> 'result-cards');

CREATE POLICY "Result cards cannot be modified by clients"
  ON storage.objects FOR UPDATE TO anon, authenticated
  USING (bucket_id <> 'result-cards') WITH CHECK (bucket_id <> 'result-cards');

CREATE POLICY "Result cards cannot be deleted by clients"
